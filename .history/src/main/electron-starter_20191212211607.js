...
function hasActiveSubscription(customerSubscriptionId) {
    chargebee.configure({site : "escape-overseer-test", 
        ...
        //TODO: change this to customerSubscriptionId \/
    chargebee.subscription.retrieve(customerSubscriptionId).request(function(error,result) {
      if(error){
        //handle error
        console.log(error);
        loginWindow.webContents.send('updateSelectedRoomId', false, error);
      }else{
        console.log(result);
        ...
        loginWindow.webContents.send('updateSelectedRoomId', (subscription.status == 'active' || subscription.status == 'in_trial'), '');
      }
    });
}
