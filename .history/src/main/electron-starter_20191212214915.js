...
ipcMain.on("verifySubscription", (event, customerSubscriptionId) => {
    hasActiveSubscription(customerSubscriptionId, function(accepted) {
...
function hasActiveSubscription(customerSubscriptionId, callback) {
    ...
    chargebee.subscription.retrieve(customerSubscriptionId).request(function(error,result) {
      if(error){
        ...
        callback(subscription.status == 'active' || subscription.status == 'in_trial');
      }
    });
}
