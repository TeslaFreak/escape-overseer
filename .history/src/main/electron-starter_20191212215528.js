...
function createLoginWindow() {
    ...
    await function hasActiveSubscription(customerSubscriptionId) {

    try {
        chargebee.configure({site : "escape-overseer-test", 
            api_key : "test_TCwzWlKEcumk4Jdu96DZ4qZUFACR0HAPl"});

         error,result = await chargebee.subscription.retrieve(customerSubscriptionId).request(function(error,result) {
        if(error){
            //handle error
            console.log(error);
            return false;
