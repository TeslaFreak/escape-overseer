    ...
    try{
        ...
        if (result.subscription.status == 'active' || result.subscription.status == 'in_trial') {
            let freshToken = {
                ...
                expiration: result.subscription.status == 'active' ? result.subscription.current_term_end : result.subscription.trial_end
            }
            console.log('storing auth token');
            loginWindow.webContents.send('storeAuthToken', freshToken);
        } else {
