...
function hasActiveSubscription(customerSubscriptionId) {
    ...
    chargebee.subscription.retrieve(customerSubscriptionId).request(function(error,result) {
      if(error){
        //handle error
        console.log(error);
        loginWindow.webContents.send('verifySubscriptionResponse', false, error);
      }else{
        console.log(result);
        ...
        loginWindow.webContents.send('verifySubscriptionResponse', (subscription.status == 'active' || subscription.status == 'in_trial'), '');
      }
    });
}
