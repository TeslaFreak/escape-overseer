...
class ClueCountEditPanel extends Component {
    ...
    constructor(props) {
        super(props);
        this.state={iconSize: parseInt(this.props.selectedItem.iconSize**(1/1.7), 10),
                    iconSpacing: parseInt(this.props.selectedItem.iconSpacing**(1/2.1), 10)};
        this.objects = [];
    ...
    componentWillUpdate(nextProps) {
        ...
        if (this.state.iconSpacing !== parseInt(nextProps.selectedItem.iconSpacing**(1/2.1), 10) ) {
            this.setState({iconSpacing: parseInt(nextProps.selectedItem.iconSpacing**(1/2.1), 10)})
        }
        if (this.state.iconSize !== parseInt(nextProps.selectedItem.iconSize**(1/1.7), 10) ) {
            this.setState({iconSize: parseInt(nextProps.selectedItem.iconSize**(1/1.7), 10)})
        }
    }
    ...
    handleChange = (event, value, propertyName) => {
        ...
        switch(propertyName) {
            case 'iconSize':
                ...
                break;
            case 'iconSpacing':
                roundedValue = parseInt(value, 10)**2.1;
                displayValue = parseInt(value, 10);
                break;
            default:
                roundedValue = value;
