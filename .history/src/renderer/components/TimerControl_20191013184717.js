...
class TimerControl extends React.Component{
  constructor(props) {
    super(props);
    this.state = {minutes: this.props.totalTime,
                  seconds: 0,
                  milliseconds: 0,
                  paused: true,
                  timeToAdjust: 0};
  ...
  componentWillUpdate(nextProps){
    if(nextProps.totalTime !== this.props.totalTime){
        this.setState({minutes:nextProps.totalTime});
    }
  }
  ...
  componentDidUpdate() {
    electron.ipcRenderer.send('updateLiveViewTimeDisplay', this.state.minutes, this.state.seconds, this.state.milliseconds);
  }

  tick() {
    var sec = this.state.seconds;
    var min = this.state.minutes;
    var millisec = this.state.milliseconds;

    if(this.state.paused == true) {
      return;
    }
    else if (min <= 0 && sec <= 0 && millisec <=0) {
      ...
      this.setState({seconds: sec, minutes: min, milliseconds: millisec});
    }
  }
  ...
  resetTimer = () => {
    this.setState({paused: true, minutes: this.props.totalTime, seconds:0, milliseconds: 0, timeToAdjust: 0});
  }
  ...
  timeIsUp() {
    return (this.state.minutes <=0 && this.state.seconds <= 0 && this.state.milliseconds <=0);
  }
