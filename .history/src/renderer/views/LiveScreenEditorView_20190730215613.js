    ...
    updateItemProperty = (propertyName, propertyValue) => {
        ...
        switch(propertyName) {
            ...
            case 'iconSpacing':
                ...
                break;
            case 'numberOfClues':
                var groupWidth = (this.state.selectedItem.iconSpacing*(propertyValue-1))+(iconSize*propertyValue);
                fabric.loadSVGFromURL("assets/images/lock-solid.svg", function(object) {
                }, function(item, object) {
                    tmpObj = object.set({ left: -(groupWidth / 2) + (this.state.selectedItem.iconSpacing+this.state.selectedItem.iconSize)*propertyValue});
                    tmpObj.scaleToWidth(this.state.selectedItem.iconSize);
