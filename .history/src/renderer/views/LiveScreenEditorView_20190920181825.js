...
class LiveScreenEditorView extends Component {
    ...
    updateItemProperty = (propertyName, propertyValue) => {
        ...
        switch(propertyName) {
            ...
            case 'fontFamily':
                ...
                break;
            case 'aspectRatio':
                //TODO: fix resizing problem. if you flip dominant dimension, then go to a different ar, it doesnt update size till you resize window
    ...
    createNewCanvasItem = (itemType, event) => {
        switch(itemType) {
            ...
            case CanvasItemTypes.TIMER:
                ...
                break;
            case CanvasItemTypes.VISUALCOUNTER:
                if(this.canvas.getObjects('numericCounter') != [] && this.canvas.getObjects('visualCounter') != []) {
                    console.log(this.canvas.getObjects('numericCounter'));
                    console.log(this.canvas.getObjects('visualCounter'));
                    alert('Only one counter allowed at a time');
                    return
                }
                ...
                break;
            case CanvasItemTypes.NUMERICCOUNTER:
                    if(this.canvas.getObjects('numericCounter') != null && this.canvas.getObjects('visualCounter') != null) {
                        alert('Only one counter allowed at a time');
                        return
                    }
    ...
    //TODO: find timer object and save time separately.
    //TODO: find clue counter and save clue count separately.
    ...
    saveJSON = async () => {
      ...
      this.db.get(this.props.selectedRoomId + '\\liveScreen').then(function (doc) {
        ...
        doc.canvasDominantDimension=this.state.aspectDominantDimension;
        this.db.put(doc).catch(function (err) {
            ...
            if(err.name=="not_found") {
                this.db.put({
                    ...
                    canvasDominantDimension: this.state.aspectDominantDimension
                }).catch(function (err) {
                    console.log(err);
                });
            }
