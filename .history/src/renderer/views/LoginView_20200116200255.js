...
export default function SignIn() {
  ...
  const handleLoginAttempt = async (e) => {
    ...
    if(navigator.onLine) {
        ...
        let result = await fetch('https://21731c69.us-south.apigw.appdomain.cloud/escapeoverseerAPI/verifyUserCredentials', {
                    method: 'POST',
                    headers: new Headers( {'Content-Type': 'application/json'}),
                    body: JSON.stringify({"credentials": {"email": 'doesnt matter', "password": 'matters not'}})
                });
        if (!result.success) {
            console.log(result);
        }
