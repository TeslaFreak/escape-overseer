...
class LiveScreenEditorView extends Component {
    ...
    componentDidMount() {
        ...
        fabric.prototype.loadAmbiguousFromURL = function(url, callback) {
    ...
    renderEditPanels = (selectedPanel) => {
        switch(selectedPanel) {
            ...
            case EditPanelTypes.COUNTER:
                return <CounterEditPanel selectedItem={this.state.selectedItem} updateItemProperty={this.updateItemProperty} canvasObjectCount={this.canvas.size()}/>;
            default:
        ...
        switch(propertyName) {
            ...
            case 'numberOfClues':
                ...
                if (oldClueCount < propertyValue) {
                    for (var i = Number(oldClueCount)+1; i <= propertyValue; i++) { 
                        fabric.loadAmbiguousFromURL("assets/images/lock-solid.svg", function(object) {
                            ...
                            this.updateItemProperty('fill', this.state.selectedItem.fill);
            ...
            case 'changeSrc':
                //let filetype = propertyValue.slice((filename.lastIndexOf(".") - 1 >>> 0) + 2);
                    this.state.selectedItem.forEachObject(function(obj, i) {
                        obj.setSrc('assets/images/baseline-lock-24px.svg', function(newImg) {
                            newImg.set('width', obj.width);
                            newImg.set('height', obj.height);
                            //dumb fix for group not rendering after updating clue count
                            this.updateItemProperty('iconSpacing', this.state.selectedItem.iconSpacing);
                        });
                    });
    ...
    createNewCanvasItem = (itemType, event) => {
        switch(itemType) {
            ...
            case CanvasItemTypes.COUNTER:
                ...
                fabric.CounterImage.fromURL("assets/images/lock-solid.svg", function(object) {
                    tmpObj = object.set({
                        left: 0
                    });
                    tmpObj.scaleToWidth(12);
                    objWidth = 12;
                    group.push(tmpObj);
                    fabric.Image.fromURL("assets/images/lock-solid.svg", function(object) {
                        tmpObj = object.set({
