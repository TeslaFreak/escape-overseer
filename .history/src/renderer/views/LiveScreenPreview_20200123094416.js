...
class LiveScreenPreview extends React.PureComponent {
  ...
  updateBackgroundImage() {
    this.db.getAttachment(electron.remote.getGlobal('customerEmail') + '/' + this.props.selectedRoomId + '/backgroundImg', 'backgroundImgFile').then(function(blob) {
  ...
  playVideo = () => {
    ...
    return this.db.getAttachment(electron.remote.getGlobal('customerEmail') + '/' + this.props.selectedRoomId + '/breifVideo', 'breifVideoFile').then(function(blob) {
        var url = URL.createObjectURL(blob);
