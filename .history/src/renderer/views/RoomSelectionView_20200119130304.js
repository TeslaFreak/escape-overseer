...
class RoomSelectionView extends Component {  
  constructor(props) {
    ...
    this.companyName = electron.remote.getGlobal('companyName');
  ...
  openNewRoomModal = () => {
    this.setState({newRoomModalOpen: true});
  }
  ...
  refreshRoomList = async () => {
      try {
        const doc = await this.db.get(this.companyName + ':rooms');
  ...
  saveRoom = (name, id) => {
    ...
    this.db.get(this.companyEmail + ':rooms').then(function (doc) {
  ...
  //TODO:[V1.1 Prefered] When room is deleted, go through and delete all records associated with room
  // right now we are only deleteing it from the list, but other records such as canvas info are still
  // taking up space in DB
  deleteRoom = (id) => {
    this.db.get(this.companyEmail + ':rooms').then(function (doc) {
        let roomlist = doc.roomList.filter((value,index,array)=>{
            return value._id != id;
        });
        doc.roomList = roomlist;
        this.db.put(doc).then(function() {
  ...
  render() {
    ...
    return (
        ...
        <Popover 
            ...
            >
            <MenuList>
                <MenuItem onClick={()=>{this.setState({changeNameModalOpen:true, anchorEl:null})}} align='start' size='small' >Change Room Name</MenuItem>
                <MenuItem onClick={()=>{this.setState({deleteRoomModalOpen:true, anchorEl:null})}} align='start' size='small' >Delete Room</MenuItem>
            </MenuList>
        </Popover>
