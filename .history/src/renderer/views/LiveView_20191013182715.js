...
class LiveScreen extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    fabric.ClueTextbox = fabric.util.createClass(fabric.Textbox, {
        ...
        updateClueText: function(liveText) {
            this.set("text", liveText);
        }
    });
    ...
    fabric.Timer = fabric.util.createClass(fabric.IText, {
        ...
        //TODO:[V1 Preferable] fix to show milliseconds correctly. Also test with all timer formats
        updateTimeDisplay: function(liveMinutes, liveSeconds, liveMillsec) {
            console.log(liveMillsec)
            if(liveMinutes <=0 && liveSeconds <= 0 && liveMillsec <= 0) {
                this.set("text", "Game Over!");
                return;
            }
            let remainingMinutes = (liveMinutes || liveMinutes === 0) ? liveMinutes : this.totalTime;
            let displayString = '';
            if(this.showHours) {
            ...
            if(this.showSeconds) {
                let totalSeconds = remainingMinutes*60 + liveSeconds;
            ...
            if(this.showMilliseconds) {
                let totalMilliseconds = remainingMinutes*60*1000 + liveMillsec;
  ...
  render() {
    ...
    electron.ipcRenderer.on('updateLiveViewTimeDisplay', (event, min, sec, millisec) => {
        console.log(millisec);
      this.setState({minutes: min, seconds: sec, milliseconds: millisec},
        this.timer.updateTimeDisplay(Number(min),(Number(sec)),Number(millisec)));
