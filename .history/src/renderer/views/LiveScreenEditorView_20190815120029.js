    ...
    updateItemProperty = (propertyName, propertyValue) => {
        ...
        switch(propertyName) {
            ...
            case 'usedStatus':
                ...
                this.state.selectedItem.forEachObject(function(obj, i) {
                    if(i%2 == 0) {
                        obj.set({visible: propertyValue == 'unused'})
                    }
                    else{
                        obj.set({visible: propertyValue == 'used'})
                    }
                }.bind(this));
                break;
            case 'fontFamily':
            ...
            case 'changeSrc':
                ...
                this.state.selectedItem.set('unusedSource', 'assets/images/ex-solid.png');
                    this.state.selectedItem.forEachObject(function(obj, i) {
                        obj.setSrc('assets/images/ex-solid.png', function(newImg) {
                            //dumb fix for group not rendering after updating clue count
    ...
    createNewCanvasItem = (itemType, event) => {
        switch(itemType) {
            ...
            case CanvasItemTypes.COUNTER:
                    ...
                    newItem.updateItemProperty('numberOfClues', 3);
                    newItem.on('modified', () => {
                        this.updateSelectedItem(newItem, itemType);
                    });
                    newItem.on('selected', () => {
