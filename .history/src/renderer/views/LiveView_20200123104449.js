...
class LiveScreen extends React.PureComponent {
    ...
    fabric.Timer = fabric.util.createClass(fabric.IText, {
        ...
        updateTimeDisplay: function(liveMinutes, liveSeconds) {
            ...
            if(this.showMilliseconds) {
                let totalMilliseconds = remainingMinutes*60*1000 + liveSeconds*1000;
  ...
  loadJSON = async () => {
      this.db.get(electron.remote.getGlobal('customerEmail') + '/' + this.state.selectedRoomId + '/liveScreen').then(function(doc) {
  ...
  playVideo = () => {
    ...
    return this.db.getAttachment(electron.remote.getGlobal('customerEmail') + '/' +  this.state.selectedRoomId + '/breifVideo', 'breifVideoFile').then(function(blob) {
  ...
  closeVideo = () => {
    ...
    electron.ipcRenderer.send('startTimer');
  }
