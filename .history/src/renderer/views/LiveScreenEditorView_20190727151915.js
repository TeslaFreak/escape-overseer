        ...
        switch(propertyName) {
            ...
            case 'iconSize':
                    ...
                    this.state.selectedItem.set({
                        scaleX: this.state.selectedItem.scaleX * propertyValue,
                        scaleY: this.state.selectedItem.scaleY * propertyValue
                    });
                    this.state.selectedItem.set({ iconSize: propertyValue });
                ...
                case 'iconSpacing':
                    ...
                    var itemLen = this.state.selectedItem._objects.length;
                    var iconSpacing = this.state.selectedItem.iconSpacing;
    
                    this.state.selectedItem.forEachObject(function(obj, i) {
                        if(i != 0) {
                            obj.set({
                                left: obj.left + (propertyValue - iconSpacing) * i * scalePos
                            });
                        }
                    });
                    this.state.selectedItem.set({
                        width: this.state.selectedItem.width + 
                            (propertyValue - this.state.selectedItem.iconSpacing) * (itemLen - 1) * scalePos,
                            iconSpacing: propertyValue
                    });
