        ...
        fabric.Timer = fabric.util.createClass(fabric.IText, {
            ...
            updateTimeDisplay: function() {
                ...
                if(this.showHours) {
                    ...
                    displayString += this.showMinutes ? `${totalHours}:` : `${totalHours}`;
                    remainingMinutes %= 60;
                }
                if(this.showMinutes) {
                    let totalMinutes = remainingMinutes;
                    displayString += this.showSeconds ? `${totalMinutes}:` : `${totalMinutes}`;
                    remainingMinutes -= totalMinutes;
                }
                if(this.showSeconds) {
                    let totalSeconds = remainingMinutes*60;
                    displayString += this.showMilliseconds ? `${totalSeconds}:` : `${totalSeconds}`;
                    remainingMinutes -= remainingMinutes*60;
                }
                if(this.showHours) {
                    let totalMilliseconds = remainingMinutes*60*100;
                    displayString += `${totalMilliseconds}`;
                }
