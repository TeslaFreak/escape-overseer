...
class LiveScreenEditorView extends Component {
    ...
    renderEditPanels = (selectedPanel) => {
        switch(selectedPanel) {
            ...
            case EditPanelTypes.VIDEO:
                return <VideoEditPanel selectedItem={this.state.selectedItem} updateItemProperty={this.updateItemProperty}/>;
        ...
        this.db.get(this.props.selectedRoomId + '\\breifVideo').then(function (doc) {
            this.db.remove(doc).then(function () {
                this.db.put({
                  ...
                  }).then(console.log('done1')).catch(function (err) {
                    console.log(err);
                  });
            }.bind(this));
            ...
            if(err.name == 'not_found') {
                this.db.put({
                  ...
                  }).then(console.log('done2')).catch(function (err) {
                    console.log(err);
                  });
            }
        }.bind(this));
    ...
    render() {
        ...
        //TODO:[V1 Mandatory] Add place to upload intro videos. Probably a Misc. edit panel for whole canvas, below aspect nav button
        return(
