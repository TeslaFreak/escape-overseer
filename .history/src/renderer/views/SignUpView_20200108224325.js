    ...
    const handleSignUpAttempt = (e) => {
        ...
        if (navigator.onLine) {
            ...
            if (validationResult) {
                ...
                (async () => {
                    const rawResponse = await fetch('https://21731c69.us-south.apigw.appdomain.cloud/escapeoverseerAPI/createEOProfile', {
                    method: 'POST',
                    headers: JSON.stringify({ "email": email, "password": document.getElementById('password').value })
                }).then((res) => res.json())
