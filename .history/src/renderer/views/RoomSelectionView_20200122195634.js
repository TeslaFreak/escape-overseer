...
class RoomSelectionView extends Component {  
  constructor(props) {
    ...
    this.companyEmail = electron.remote.getGlobal('customerEmail')
  ...
  refreshRoomList = async () => {
    ...
    const forEachRoom = async (room) => {
        try {
            const blob = await this.db.getAttachment(electron.remote.getGlobal('customerEmail') + '\\' + room._id + "\\backgroundImg", "backgroundImgFile")
            console.log(blob)
