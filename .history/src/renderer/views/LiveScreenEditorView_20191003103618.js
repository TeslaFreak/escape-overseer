...
class LiveScreenEditorView extends Component {
    ...
    loadJSON = async () => {
        this.db.get(this.props.selectedRoomId + '\\liveScreen').then(function(doc) {
            ...
            await this.updateAspectRatio(doc.canvasAspectRatio);
            setTimeout(function(){
                console.log('fired from loadJSON');
                window.dispatchEvent(new Event('resize'));
            }, 1);
            await this.canvas.loadFromJSON(doc.canvasJSON,
            await this.updateSelectedItem(null, CanvasItemTypes.SCREEN));
